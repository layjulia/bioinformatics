...
Using the frequencies stored in `ecoli_nt_freq` and `aaceti_nt_freq`, we create barplots to visualise the nucleotide composition.
...
##### Side-by-side comparison of amino acid proportions
Using `ggplot2` we do a side-by-side comparison of the two amino acid compositions. While both organisms display broadly similar distributions, we can see A. aceti shows higher proportions of alanine (A), glycine (G), proline (P), arginine (R), and serine (S), whereas E. coli is enriched in isoleucine (I), lysine (K), asparagine (N), and glutamine (Q).
...
To compare the two organisms' nucleotides and amino acid sequences side-by-side, we use `knitr` to generate a clean html table.
...
We previously translated the CDS to protein sequences: `ecoli_prot` and `aaceti_prot`. K-mer is simply a short sequence string of length k. We can use k-mer analysis to identify particular protein sequences that are over- or under-represented in an organism. We can use this to count and then we convert counts to frequency so the different proteome sizes don't bias the comparison.
```{r}
#Prepare file
