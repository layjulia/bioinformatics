...
It looks like the mean growth over the last 10 years at each site is:
...
What the below code chunk does:
...
plot_kmer_comparison_split <- function(prot1, prot2, aa1, aa2, k = 3, top_n = 10,
  ...
  freq_matrix_over <- t(as.matrix(over_kmers[, c("Org1.Freq", "Org2.Freq")]))
  ...
  freq_matrix_under <- t(as.matrix(under_kmers[, c("Org1.Freq", "Org2.Freq")]))
...
```
We can quantify the changes using a table with:
k-mer
frequency in A. aceti
frequency in E. coli
fold-change (A. aceti /E. coli) - check for div by 0
```{r}
#create function
get_kmer_summary <- function(prot1, prot2, aa1, aa2, k = 3, top_n = 10,
  ...
  kmer_table$FoldChange <- ifelse(kmer_table$Org2.Freq > 0,
                                  kmer_table$Org1.Freq / kmer_table$Org2.Freq,
                                  NA)
