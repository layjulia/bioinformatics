...
Note that the `count()` function must be distinguished as it is in both `seqinr` and `dplyr`
```{r loading packages}
...
seqinr::count(ecoli_prot,wordsize=1,alphabet=aa)
seqinr::count(aaceti_prot,wordsize=1,alphabet=aa1)
```
...
However, if we could group the codons by the amino acids we can further visualise the most common ones with the codon bias for each. The stacked bar graph is clearer in showing the contribution of the codons to a specific amino acid.
...
For the top 10 highest and 10 lowest frequency kmers in A. aceti, we compare the frequencies with E. coli - i.e. for the same k-mers
```{r}
# Add side-by-side bars
barplot(
  t(as.matrix(aaceti_top_k3[, c("Aaceti.Freq","Ecoli.Freq")])),
  beside = TRUE,
  ...
  ylab = "Frequency",
  xlab = "3-kmers",
  main = "Comparison of 3-kmers: A. aceti vs E. coli"
)
legend("topright", legend=c("A. aceti","E. coli"), fill=c("plum1","skyblue"))

#4-mers
barplot(
  t(as.matrix(aaceti_top_k4[, c("Aaceti.Freq","Ecoli.Freq")])),
  beside = TRUE,
  names.arg = aaceti_top_k4$Aaceti.Var1,
  col = c("plum1", "skyblue"),
  las = 2,
  ylab = "Frequency",
  xlab = "4-kmers",
  main = "Comparison of 4-kmers: A. aceti vs E. coli"
)
legend("topright", legend=c("A. aceti","E. coli"), fill=c("plum1","skyblue"))

#5-mers plot
barplot(
  t(as.matrix(aaceti_top_k5[, c("Aaceti.Freq","Ecoli.Freq")])),
  beside = TRUE,
  names.arg = aaceti_top_k5$Aaceti.Var1,
  col = c("plum1", "skyblue"),
  las = 2,
  ylab = "Frequency",
  xlab = "5-kmers",
  main = "Comparison of 5-kmers: A. aceti vs E. coli"
)
legend("topright", legend=c("A. aceti","E. coli"), fill=c("plum1","skyblue"))
```

Using a function to summarise the above more efficiently
```{r}
# Function to get top over/under-represented k-mers and plot:
plot_kmer_comparison <- function(prot1, prot2, aa1, aa2, k = 3, top_n = 10,
  ...
  kmer_table <- data.frame(
    ...
    Org1.Freq = as.numeric(kmers1_freq),
    Org2.Freq = as.numeric(kmers2_freq),
    stringsAsFactors = FALSE
  )
  
  # Select top and bottom k-mers in organism 1
  over_kmers  <- head(kmer_table[order(-kmer_table$Org1.Freq), ], top_n)
  under_kmers <- head(kmer_table[order(kmer_table$Org1.Freq), ], top_n)
  
  # Combine
  top_kmers <- rbind(over_kmers, under_kmers)
  
  # Create matrix for barplot
  freq_matrix <- t(as.matrix(top_kmers[, c("Org1.Freq", "Org2.Freq")]))
  colnames(freq_matrix) <- top_kmers$Kmer
  ...
  barplot(
    ...
    main = paste("Over/Under-represented", k, "-mers in", org1_name, "vs", org2_name),
    cex.names = 0.8
  )
...
plot_kmer_comparison(aaceti_prot, ecoli_prot, aa1, aa, k=3)
plot_kmer_comparison(aaceti_prot, ecoli_prot, aa1, aa, k=4)
plot_kmer_comparison(aaceti_prot, ecoli_prot, aa1, aa, k=5)
