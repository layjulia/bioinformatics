...
##### How many coding sequences are present in these organisms? Present this in the form of a table. 
We can figure out the number of coding sequences (CDS) and present this in a table called `cds_table`. We can use `diff` to calculate the difference of the `CDS_Count` column.
...
##### Calculate the length of all coding sequences in these two organisms.
We can calcuate the mean and median coding sequences then store it all in a data frame. Again, `diff` is used to show the difference between the means and medians.
...
We can also create a boxplot for the CDS lengths of both organisms.
...
Using the frequencies stored in `ecoli_nt_freq` and `aaceti_nt_freq`, we can create barplots to visualise the nucleotide composition.

```{r}
#Barplot for nucleotide frequencies of E. coli
...
barplot(ecoli_aa_count,
        ...
        cex.axis = 0.75, # Changes the size of axis tick mark annotations
        cex.lab = 0.75, # Changes the size of x-axis and y-axis labels
        cex = 0.75,
...
* This indicates that A. aceti has a higher GC content compared to E. coli, which could potentially influence genome stability, codon usage (explored later in question 5) and possibly its gene expression.
...
Before, we translated the CDS to protein sequences. K-mer is simply a short sequence string of length k. We can use k-mer analysis to identify particular protein sequences that are over- or under-represented in an organism. We counted unique amino acids in ecoli earlier and stored as `aa` (E. coli) and `aa1` (A. aceti). We can use this to count and then we convert counts to frequency so the different proteome sizes don't bias the comparison.
```{r}
#Prepare file
...
The frequencies can be placed into a table together.
```{r}
...
We can also create a ranking column to label the top/bottom A. aceti and E. coli kmers, then see how they compare to each other.
The ranking of both organisms mean we can create rank-rank scatterplots.
```{r}
...
We can then get the top 10 highest and lowest frequency kmers and confirm this with the ranking column. We can also view where E. coli's corresponding kmers rank. `knitr::kable` is a table generator for rectangular data.
```{r}
#for k=3
...
# table for k=3
knitr::kable(
  k3_summary[, c("Type", "Aaceti.Var1", "Aaceti.Freq", "Aaceti.rank", "Ecoli.rank")], 
...
# table for k=4
knitr::kable(
  k4_summary[, c("Type", "Aaceti.Var1", "Aaceti.Freq", "Aaceti.rank", "Ecoli.rank")], 
...
# table for k=5
knitr::kable(
  k5_summary[, c("Type", "Aaceti.Var1", "Aaceti.Freq", "Aaceti.rank", "Ecoli.rank")], 
...
The benefit of ranking the frequencies earlier is that we can now create rank/rank scatterplots. A rank/rank scatterplot is a great way to see how similar or different the k-mer rankings are between A. aceti and E. coli. We will generate three scatterplots for kmers length of 3, 4 and 5.

This is the rank/rank scatterplot for 3-mer. Points close to the red line means similar ranking in both organisms. For example, if rank 1 of E. coli is the same motif as rank 1 of A. aceti, then they would be on the red line. Outliers from the red line mean very different usage between organisms.
```{r}
